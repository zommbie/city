...
{   
    ...
    protected override void OnEffectStartPosition(Vector3 vecPosition, Vector3 vecDirection, params object[] aParams)
    {
    ...
    protected override void OnEffectTimeScale(float fTimeScale)
    {
        ...
        {
            ParticleSystem.MainModule rParticleMain = m_listParticleSystem[i].main;
            rParticleMain.duration = rParticleMain.duration * fTimeScale;
        }
    }
    ...
    private void UpdeteEffectParticleExpire()
	{
		...
		for (int i = 0; i < m_listParticleSystem.Count; i++)
		{
			if (m_listParticleSystem[i].isPlaying || m_listParticleSystem[i].main.loop)
			{
				bExpire = false;
				break;
			}
