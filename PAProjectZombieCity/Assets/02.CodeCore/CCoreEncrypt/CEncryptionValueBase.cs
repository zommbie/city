...
public abstract class CEncryptionValueRootBase 
{
    ...
    public EEncryptValueType ValueType { get { return m_eValueType; } }
}

public abstract class CEncryptionValueBase<TYPE> : CEncryptionValueRootBase where TYPE : struct
{
...
public class EnULong : CEncryptionValueBase<ulong>
{
    public const byte DefaultCryptionBufferSize = 32; // 32바이트  uint64  4개 규모
    public EnULong() : base(DefaultCryptionBufferSize) { SetEncrytValueType(EEncryptValueType.ULong); }
    public EnULong(ulong iValue) : base(DefaultCryptionBufferSize) { SetEncrytValueType(EEncryptValueType.ULong); Value = iValue; }
    public ulong Value
    {
        set
        {
        ...
        get
        {
            return PrivDecryptDataULong();
        }
    }

    static public implicit operator ulong(EnULong pEnULong)
    {
        return pEnULong.Value;
    }
    //-----------------------------------------------------
    private ulong PrivDecryptDataULong()
    {
        return ProtDecryptDataStart(0, Marshal.SizeOf(typeof(ulong)));
    }
}
