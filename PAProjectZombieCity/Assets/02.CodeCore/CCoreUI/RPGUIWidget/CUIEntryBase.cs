...
public abstract class CUIEntryBase : CMonoBase
{
    ...
    public Vector2 GetUIPivot()
    {
        return m_pRectTransform.pivot;
    }
    ...
    public Vector3 GetUIPositionWorld()
    {
        Vector2 vecScreenPosition = RectTransformUtility.WorldToScreenPoint(CManagerUIFrameBase.Instance.GetUIManagerCamara(), transform.position);       
        return Camera.main.ScreenToWorldPoint(vecScreenPosition);
    }

    public Rect GetUIRectWorld()
    {       
        return GetUIRectWorld(m_pRectTransform);
    }

    public static Rect GetUIRectWorld(RectTransform pRectTransform)
    {
        Vector2 vecSize = Vector2.Scale(pRectTransform.rect.size, pRectTransform.lossyScale);
        return new Rect((Vector2)pRectTransform.position - (vecSize * 0.5f), vecSize);
    }

    public static Vector3 ConverWorldPositionToUIPosition(Vector3 vecWorldPosition)
    {
        Vector3 vecViewportPoint = Camera.main.WorldToViewportPoint(vecWorldPosition);
        Vector3 vecScreenSize = CManagerUIFrameBase.Instance.GetUIScreenSize();
        return new Vector2((vecViewportPoint.x * vecScreenSize.x) - (vecScreenSize.x / 2f), (vecViewportPoint.y * vecScreenSize.y) - (vecScreenSize.y / 2f)); 
    }
