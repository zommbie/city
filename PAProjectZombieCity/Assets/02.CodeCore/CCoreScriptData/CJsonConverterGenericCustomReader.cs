...
public class CJsonConverterGenericCustomReader : JsonConverter
{
    ...
    private void PrivJsonConverterReadValueString(JToken pReadToken, FieldInfo pFieldInfo, object pInstance) 
    {
        string strValue = pReadToken.Value<string>(); 
        ...
        else if (pFieldInfo.FieldType.IsEnum) 
        {
            strValue = strValue.Replace(' ', '_');
            Type pFieldType = pFieldInfo.FieldType; 
            object pEnumObject = Enum.Parse(pFieldType, strValue);                                                                    
            pFieldInfo.SetValue(pInstance, pEnumObject); 
        }
    }
