...
abstract public class CScriptDataBase : CMonoBase
{
    ...
    [SerializeField]
    private bool Kor949 = false; // 코드 페이지 949(멀티바이트 한글 페이지) 사용할 경우 l18N.CJK.dll / l18N.dll 을 플러그인 해야한다 (실행파일 1MB 증가 페널티)
	//------------------------------------------------------------

	public void DoScriptDataLoad(byte[] aTextData)
	{
    ...
    public void DoScriptDataLoad(string strTextData)
    {
        OnScriptDataLoad(strTextData);
        OnScriptDataLoadFinish();
    }
	...
	//------------------------------------------------------------
	internal void InterScriptDataLoad()
	{
