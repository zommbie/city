...
{       
	...
	{        
        ...
        if (pAddresableInfo.DependencyAssetBundle.Count == 1)  // 카운트 1일 경우만 해재 (Seperate 로 패킹했을때)
        {
            AssetBundle pAssetBundle = pAddresableInfo.DependencyAssetBundle[0].BundleInstance;
            if (pAssetBundle != null) // 수동으로 해재가 되지 않았을 경우
            {
