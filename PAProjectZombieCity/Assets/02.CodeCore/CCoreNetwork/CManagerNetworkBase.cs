...
// 1. 네트워크 세션에 유니티 이벤트를 전달하기 위한 과정
// 2. 하나의 게임에는 복수의 세션이 존재 할 수 있다. Ex) 백앤드 세션 + 채팅 세션 + 공지 세션등
// 3. 유니티 세션등 기타 외부 네트워크 객체들에 대한 추상화를 제공한다. 
...
{   
    ...
    //-----------------------------------------------------------------------------
    private void HandleMgrGameNetworkErrorMessage(int eError, string strMessage, int iArg, float fArg)
	{
		Debug.Log(string.Format("[Network Session] ResultID : {0} / Message {1} / Arg {2} / Arg {3}", eError, strMessage, iArg, fArg));
		OnMgrNetworkSessionErrorMessage(eError, strMessage, iArg, fArg);
	}
    ...
    protected virtual void OnMgrNetworkSessionErrorMessage(int eErrorType, string strMessage, int iArg, float fArg) { }
