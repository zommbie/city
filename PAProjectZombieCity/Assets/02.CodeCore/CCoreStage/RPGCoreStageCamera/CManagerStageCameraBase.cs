...
abstract public class CManagerStageCameraBase : CManagerTemplateBase<CManagerStageCameraBase>
{
    ...
    protected override void OnManagerUISceneLoaded()
	{
        ...
        PrivStageCameraNotifyUISceneLoaded();
        GlobalManagerStageCameraLoaded();
    }
	//----------------------------------------------------------------
    ...
    //----------------------------------------------------------------
    protected CStageCameraBase FindStageCamera(int eCameraType)
    {
        CStageCameraBase pStageCamera = null;
        if (eCameraType >= 0 && eCameraType < m_listActiveCamera.Count)
        {
            pStageCamera = m_listActiveCamera[eCameraType];
        }
        return pStageCamera;
    }
    ...
    private void PrivStageCameraNotifyUISceneLoaded()
    {
        Dictionary<int, CStageCameraBase>.ValueCollection.Enumerator it = m_mapCameraInstance.Values.GetEnumerator();
        while(it.MoveNext())
        {
            it.Current.InterStageCameraUISceneLoaded();
        }
    }
