...
{
    public static float g_TimeScale = 1f;
	...
	public float currentTimeValue
	{
		get
		{
			...
			float curTimeLength = ignoreTimeScale? (Time.time - playTimeNote):(Time.unscaledTime - playTimeNote);
            curTimeLength *= g_TimeScale;
			float curValue = curTimeLength / animationTime;
	...
	void Update ()
	{
		if(isPlaying)
		{
            float fCurrentTimeValue = currentTimeValue;
            if (fCurrentTimeValue != 0 && fCurrentTimeValue != 1)
            {
	...
	void NoteTime ()
	{
        if (!ignoreTimeScale)
        {
            playTimeNote = Time.unscaledTime * g_TimeScale;
        }
        else
        {
            playTimeNote = Time.time * g_TimeScale;
        }
	}
	...
	public void SetTweenTargetTransform(Transform pTransform) 
	{
		TargetTransform = pTransform;
		PrivTweenInitialize();
	}
	public Transform GetTweenTargetTransform() 
	{
		return TargetTransform; 
	}

	//------------------------------
	private void PrivTweenInitialize()
	{
		foreach (Jun_Tween thisTween in tweens)
		{
