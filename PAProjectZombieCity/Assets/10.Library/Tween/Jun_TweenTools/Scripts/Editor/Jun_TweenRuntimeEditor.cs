...
public class Jun_TweenRuntimeEditor : Editor 
{
	...
	public override void OnInspectorGUI ()
	{
		...
		for (int i = 0; i < tweens.arraySize; i++)
		{
			...
			if(!deleteElement)
			{
                SerializedProperty thisTween_IsChild = thisTween.FindPropertyRelative("_isChild");
				...
				{
					Transform pTargetTransform = tar.GetTweenTargetTransform();
					if (pTargetTransform != null)
					{
						thisTween_TweenObject.objectReferenceValue = pTargetTransform;
					}
					else
					{
                ...
                EditorGUILayout.PropertyField(thisTween_IsChild);
